...
export abstract class BaseRestfulService {
  ...
  createClient(path: C1Endpoints | McbEndpoints,
               pathSegments?: string[],
               queryParams?: Record<string, string>,
               method?: Methods): HttpOptions {
    this.hostnameCheck();
    this.jwtTokenCheck();
    let processedPath = String(path);
    if(pathSegments) processedPath += '/' + pathSegments.map(el => encodeURI(el)).join('/');
  ...
  private hostnameCheck() {
    if (!this.hostname || this.hostname === 'undefined') throw new Error('Hostname is not set')
  }

  private jwtTokenCheck() {
    if (!this.jwtToken || this.hostname === 'undefined') throw new Error('Jwt token is not set')
  }
}
